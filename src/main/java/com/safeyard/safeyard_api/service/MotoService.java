...
public class MotoService {
    ...
    public MotoDTO create(MotoDTO dto) {
        ...
        if (repository.existsByPlacaIgnoreCaseAndIdNot(placa, -1L)) {
            throw new IllegalArgumentException("Ja existe outra moto com a placa " + placa);
        }
        if (repository.existsByChassiIgnoreCaseAndIdNot(chassi, -1L)) {
            throw new IllegalArgumentException("Ja existe outra moto com o chassi " + chassi);
        }
    ...
    public MotoDTO update(Long id, MotoDTO dto) {
        ...
        if (novaPlaca != null && !novaPlaca.equalsIgnoreCase(moto.getPlaca())) {
            if (repository.existsByPlacaIgnoreCaseAndIdNot(novaPlaca, id)) {
                throw new IllegalArgumentException("Ja existe outra moto com a placa " + novaPlaca);
            }
        ...
        if (novoChassi != null && !novoChassi.equalsIgnoreCase(moto.getChassi())) {
            if (repository.existsByChassiIgnoreCaseAndIdNot(novoChassi, id)) {
                throw new IllegalArgumentException("Ja existe outra moto com o chassi " + novoChassi);
            }
    ...
    public String salvarImagem(Long id, MultipartFile file) {
        ...
        if (file == null || file.isEmpty()) throw new IllegalArgumentException("Arquivo nao enviado.");
        ...
        if (contentType == null || !ALLOWED_MIME.contains(contentType.toLowerCase())) {
            throw new IllegalArgumentException("Tipo de arquivo nao permitido. Use JPG ou PNG.");
        }

        Path root = Paths.get(uploadRoot, "motos");
        try { Files.createDirectories(root); }
        catch (IOException e) { throw new RuntimeException("Falha ao preparar diretorio de upload.", e); }
        ...
        if (!dest.startsWith(root.toAbsolutePath())) throw new SecurityException("Caminho de arquivo invalido.");
